...
* **OneDrive removal** using stock uninstaller, its folder in Explorer sidebar will also be hidden. Furthermore, its automatic setup will be disabled to prevent the app from being installed for new users.
...
* `UWPAppsRemovalMode`: allows you to decide whether to remove UWP apps for all present and future users (*"AllUsers"*, which is the default) or just for the current user (*"CurrentUser"*). Take note that trying to remove system apps only for the current user will result in an error.
